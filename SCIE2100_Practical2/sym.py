...
Protein_Alphabet.annotateAll('html-color',
                             {'G': 'green', 'C': 'green', 'P': 'green', 'S': '#66bbff', 'T': '#66bbff', 'H': 'red',
...
class TupleStore(dict):
    ...
    def __init__(self, alphas=None, entries=None, sparse=True):
        ...
        for idx in range(self.keylen):
            ...
            if self.alphas is not None:  # if specified it needs to be a superset of that we constructed
        ...
        if self.alphas is None:  # if not specified to constructor use those we found
