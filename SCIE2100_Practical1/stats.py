...
def getFETpval(a1, a2, b1, b2, left=True):
    """ Computes Fisher's exact test based on a null-hypothesis distribution specified by the totals, and
    an observed distribution specified by b1 and b2, i.e. determines the p-value of b's outcomes 1 and 2.
    The default setting is to use the "left" side of the density to determine the p-value.
...
def getFET2tail(a1, a2, b1, b2):
    """Computes Fisher's exact test based on a null-hypothesis distribution specified by the totals, and
    an observed distribution specified by b1 and b2, i.e. determines the two-tailed p-value of b's outcomes 1 and 2.
...
def getFETprob(a1, a2, b1, b2):
    """Computes Fisher's exact test based on a null-hypothesis distribution specified by the totals, and
    an observed distribution specified by b1 and b2, i.e. determines the probability of b's outcomes 1 and 2.

    Returns an immutable list consisting of the exact probability,
    and assorted p-values (sless, sright, sleft, slarg) based on the density.
    """
    sless = 0.0
    sright = 0.0
    sleft = 0.0
...
def hyper0(n11i, n1_i, n_1i, ni):
    ...
    if not ((n1_i | n_1i | ni) != 0):
        if not (n11i % 10 == 0):
            if n11i == _sn11 + 1:
                _sprob = _sprob * ((_sn1_ - _sn11) / float(n11i)) * (
                            (_sn_1 - _sn11) / float(n11i + _sn - _sn1_ - _sn_1))
                _sn11 = n11i
                return _sprob
            if n11i == _sn11 - 1:
                _sprob = _sprob * ((_sn11) / float(_sn1_ - n11i)) * (
