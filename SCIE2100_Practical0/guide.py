...
class Alphabet:
    ...
    def __iter__(self):  # method that allows us to iterate over all symbols,
        # e.g. "for sym in Alphabet('ACGT'): print sym" prints A, C, G and T on separate lines
...
class Sequence:
    """ A biological sequence class. Stores the sequence itself, the alphabet and a name.
        Usage:
...
class Alignment():
    ...
    def writeHTML(self, filename):
        ...
        fh = open(filename, 'wt')
        fh.write(
            u'<html><head><meta content="text/html; charset=ISO-8859-1" http-equiv="Content-Type">\n<title>Sequence Alignment</title>\n</head><body><pre>\n')
        ...
        if len(self.alphabet) <= 5:  # DNA or RNA
            ...
            colours = {'G': u'orange', 'P': u'orange', 'S': u'orange', 'T': u'orange', 'H': u'red', 'K': u'red',
                       'R': u'red', 'F': u'#66bbff', 'Y': u'#66bbff', 'W': u'#66bbff', 'I': u'green', 'L': u'green',
                       'M': u'green', 'V': u'green'}
        for seq in self.seqs:
...
def readFastaString(string, alphabet, gappy=False):
    """ Read the given string as FASTA formatted data and return the list of sequences contained within it. """
...
def readFastaFile(filename, alphabet):
    """ Read the given FASTA formatted file and return the list of sequences contained within it. """
    fh = open(filename)
    data = fh.read()
    fh.close()
