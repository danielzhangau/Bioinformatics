...
def readClustal(string, alphabet, ordered=True):
    """ Read a ClustalW2 alignment in the given string and return as an
    Alignment object. """

    if ordered:
        from collections import OrderedDict
        seqs = OrderedDict() # Sequence data
    else:
        seqs = {} # Sequence data
...
def readClustalFile(filename, alphabet, ordered = True):
    """ Read a ClustalW2 alignment file and return an Alignment object
    containing the alignment. """
    fh = open(filename)
    data = fh.read()
    fh.close()
    aln = readClustal(data, alphabet, ordered)
    return aln
