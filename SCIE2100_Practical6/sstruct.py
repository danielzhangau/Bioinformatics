...
prot_alpha = Protein_Alphabet
sstr_alpha = DSSP3_Alphabet
...
def markCountAbove(scores, width = 6, call_cnt = 4):
    ...
    #for i in ?:
        #if ? > ?:
            # ? # do something to cnt
        #if i >= ?: #once we've passed the positions < width...
            #if scores[i - width] > 100: # we must update our count to only reflect the current window
                # ? # do something to cnt
        #if cnt >= call_cnt:
            #for j in range(max(0, i - width + 1), i + 1): #Annotate all positions in window meeting criteria
                #above[j] = True
    return above
...
markCountAboveTest = [99, 98, 101, 104, 96, 103, 111, 198, 200, 22, 25, 29, 54, 61, 280, 121, 120, 145, 20, 168, 172,
                      20, 30, 33, 145, 156, 101, 192, 233, 10, 123, 123, 200, 81, 213, 71, 92, 104, 192, 112, 102,
                      62, 63, 123, 234, 211, 192, 10, 101, 102, 122, 143, 145, 12, 12, 43, 14, 213, 123, 324]